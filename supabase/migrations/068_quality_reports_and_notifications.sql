-- =====================================================
-- MIGRACIÓN: Sistema de Informes y Notificaciones
-- Descripción: Tablas para informes de calidad y notificaciones
-- Fecha: 2025-10-29
-- =====================================================
...
CREATE TABLE IF NOT EXISTS quality_reports (
  ...
  -- Estados: draft, pending_review, submitted, approved, rejected, published
...
CREATE INDEX idx_quality_reports_sample ON quality_reports(sample_id);
...
CREATE INDEX idx_quality_reports_created_by ON quality_reports(created_by);
...
CREATE TABLE IF NOT EXISTS notifications (
  ...
  -- Tipos: report_draft_ready, report_submitted, report_approved, report_rejected, test_reminder, etc.
  
  title VARCHAR(255) NOT NULL,
  message TEXT NOT NULL,
  
  -- Datos relacionados
  related_type VARCHAR(50), -- 'quality_report', 'quality_sample', 'quality_test'
  related_id UUID,
...
CREATE TABLE IF NOT EXISTS notification_preferences (
  ...
  -- Ejemplo: {"report_draft_ready": true, "report_submitted": true, ...}
...
-- 5. Función para generar borrador de informe automáticamente
...
BEGIN
  ...
  v_report_number := v_sample.project_code || '-CC-' || 
                     TO_CHAR(NOW(), 'YYYYMM') || '-' ||
                     LPAD(NEXTVAL('quality_reports_id_seq')::TEXT, 4, '0');
  
  -- Obtener residente asignado al proyecto (primer residente activo)
  ...
  INSERT INTO quality_reports (
    ...
    jsonb_build_object(
      'sample_info', jsonb_build_object(
        ...
        'template_name', v_sample.template_name
      ),
...
-- 6. Trigger para notificar cuando se envía un informe
...
BEGIN
  -- Solo ejecutar cuando cambia a 'submitted'
  IF NEW.status = 'submitted' AND OLD.status != 'submitted' THEN
    ...
    -- Obtener supervisor del proyecto (Santiago o cualquier supervisor)
...
-- 7. Trigger para notificar aprobación/rechazo
...
-- 8. Función para programar generación automática de borradores
-- (Se ejecutará mediante un cron job o scheduled function)
CREATE OR REPLACE FUNCTION schedule_report_drafts()
RETURNS TABLE(report_id UUID, sample_id UUID, message TEXT) AS $$
DECLARE
  v_sample RECORD;
  v_report_id UUID;
BEGIN
  -- Buscar muestras completadas que necesitan informe en 2 días
  FOR v_sample IN
    SELECT 
      qs.id,
      qs.sample_number,
      MAX(qt.test_date) as last_test_date
    FROM quality_control_samples qs
    JOIN quality_control_tests qt ON qt.sample_id = qs.id
    WHERE qs.status = 'completed'
      AND qs.overall_result IS NOT NULL
      AND NOT EXISTS (
        SELECT 1 FROM quality_reports qr
        WHERE qr.sample_id = qs.id
      )
    GROUP BY qs.id, qs.sample_number
    HAVING MAX(qt.test_date) <= CURRENT_DATE + INTERVAL '2 days'
  LOOP
    BEGIN
      v_report_id := auto_generate_report_draft(v_sample.id);
      
      IF v_report_id IS NOT NULL THEN
        report_id := v_report_id;
        sample_id := v_sample.id;
        message := 'Borrador generado para muestra ' || v_sample.sample_number;
        RETURN NEXT;
      END IF;
...
-- 9. RLS Policies para quality_reports
ALTER TABLE quality_reports ENABLE ROW LEVEL SECURITY;

-- Ver informes: supervisores ven todos, residentes solo los suyos
...
-- Crear borradores: sistema o residentes
...
-- 10. RLS Policies para notifications
...
CREATE POLICY "System can create notifications"
  ...
  WITH CHECK (TRUE); -- Solo funciones del sistema crean notificaciones

-- 11. RLS Policies para notification_preferences
...
COMMENT ON FUNCTION auto_generate_report_draft IS 'Genera borrador de informe automáticamente 2 días antes';
COMMENT ON FUNCTION schedule_report_drafts IS 'Función para ejecutar en cron job diario';
