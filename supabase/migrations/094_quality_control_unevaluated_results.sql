-- =====================================================
-- MIGRACIÓN 094: MUESTRAS SIN EVALUACIÓN AUTOMÁTICA
-- =====================================================
-- Antes, un ensayo cuyos resultados no tenían criterio de aceptación
-- evaluable (meets_criteria NULL) se contaba como aprobado y la muestra
-- terminaba en 'CUMPLE'. Ahora esos casos se marcan como 'SIN EVALUAR'
-- para que nadie los lea como conformes.
...
BEGIN
  ...
  INTO v_total_tests, v_completed_tests, v_failed_tests, v_unevaluated_tests
  FROM public.quality_control_tests qct
  ...
  ELSIF v_completed_tests = v_total_tests THEN
    ...
    SET status = 'completed',
        overall_result = CASE
          ...
          WHEN v_unevaluated_tests > 0 THEN 'SIN EVALUAR'
          ELSE 'CUMPLE'
        END
    WHERE id = v_sample_id;
  ELSE
