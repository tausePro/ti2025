-- Migración para mejorar el schema de proyectos
-- Fecha: 2025-01-27
-- Descripción: Agregar campos faltantes, tablas relacionadas y optimizaciones
...
-- 10. Crear función para calcular progreso automático
...
DECLARE
  ...
  progress INTEGER;
BEGIN
  SELECT projects.start_date, projects.end_date 
...
-- 11. Crear función para generar código de proyecto único
...
-- 12. Crear trigger para generar código automático
...
-- 13. Crear función para actualizar duración real
CREATE OR REPLACE FUNCTION update_actual_duration()
RETURNS TRIGGER AS $$
DECLARE
  start_date DATE;
  end_date DATE;
BEGIN
  IF NEW.status = 'finalizado' AND OLD.status != 'finalizado' THEN
    ...
    IF start_date IS NOT NULL THEN
      UPDATE projects 
      SET actual_duration_days = EXTRACT(DAY FROM (CURRENT_DATE - start_date))
      WHERE id = NEW.id;
    END IF;
  END IF;
...
CREATE TRIGGER trigger_update_actual_duration
  ...
  FOR EACH ROW EXECUTE FUNCTION update_actual_duration();

-- 14. Crear políticas RLS para las nuevas tablas
...
-- 15. Crear vista para proyectos con información completa
